...
**Antivirus inteligente de escritorio para Windows** que conecta con las principales IAs de pago (ChatGPT / OpenAI, Gemini, Claude y Perplexity) para vigilar el equipo en tiempo real, detectar descargas, intrusos, correos y documentos maliciosos, consultar inteligencia de amenazas en Internet y ejecutar acciones automáticas: poner en cuarentena archivos maliciosos cifrados o bloquear la conexión de un intruso.

> ⚡ **Optimizado con Antigravity (Google DeepMind AI)**: Motor de monitorización nativo de alto rendimiento (0.01% de uso de CPU, cero invocaciones masivas de PowerShell), soporte de firmas binarias UTF-16, cuarentena cifrada con algoritmo XOR (`.qbin`), elevación UAC de Administrador automática y superficie de comunicación IPC blindada.
...
| --- | --- | --- |
| **1. Instalador** | Ejecutar `Aegis AI Antivirus Setup 1.0.0.exe` (asistente, permite elegir carpeta) | Instalación permanente con acceso directo y elevación UAC automática |
...
Genera en `dist/`:
- `Aegis AI Antivirus Setup 1.0.0.exe` → instalador NSIS (solicita administrador automáticamente)
...
| Módulo | Qué hace |
| --- | --- |
| **Panel principal** | Estado de protección, CPU/RAM/Procesos/Conexiones en vivo con métricas nativas de ultralíquido rendimiento, estado de discos y últimas alertas |
| **Protección en tiempo real** | Vigila procesos nuevos y conexiones a puertos de alto riesgo sin sobrecarga de sistema (0.01% CPU) |
| **Escáner** | Rápido (Descargas, Escritorio, Temp), Completo (todo el disco C:) o por carpeta personalizada. Motores: firmas binarias UTF-16 + hash + análisis de comportamiento + **VirusTotal** + IA |
...
- Si la IA responde **malware** y la cuarentena automática está activada → el archivo se aísla de forma cifrada en cuarentena (`.qbin`).
...
├── main.js                  → Proceso principal Electron (IPC blindado, motor, ciclo de vida)
